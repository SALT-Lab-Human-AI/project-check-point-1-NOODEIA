...
// This webhook endpoint processes AI responses independently
// It's called via HTTP to avoid timeout issues
export async function POST(request) {
  try {
    const startTime = Date.now()
    const { groupId, parentMessageId, userId } = await request.json()

    console.log('🤖 Webhook AI request started:', { groupId, parentMessageId })
...
`
    ...
    console.log(`🤖 Webhook AI request completed in ${Date.now() - startTime}ms`)
